...
export class QueryBuilder<T extends Document> {
  public modelQuery: Query<(T & Document)[], T>;
  public readonly query: Record<string, string>;

  constructor(modelQuery: Query<(T & Document)[], T>, query: Record<string, string>) {
    this.modelQuery = modelQuery;
    this.query = query;
  }
  ...
  filter(): this {
    ...
    // Cast to Query<(T & Document)[], T>
    this.modelQuery = this.modelQuery.find(filter) as Query<(T & Document)[], T>;
    return this;
  }
  ...
  search(searchableField: string[]): this {
    ...
    this.modelQuery = this.modelQuery.find({ $or: conditions }) as Query<(T & Document)[], T>;
    return this;
  }
  ...
  populate(path: string | string[] | { path: string; select?: string } | any): this {
    this.modelQuery = this.modelQuery.populate(path);
    return this;
  }
  ...
  async getMeta() {
    ...
    // Get the filter query applied so far
    const filterQuery = this.modelQuery.getFilter();
